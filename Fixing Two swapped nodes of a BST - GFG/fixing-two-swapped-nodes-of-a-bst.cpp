...
class Solution {
  public:
    struct Node *correctBST(struct Node *root) {
        // code here
        vector<Node*>holder(2, NULL);
        inorder(root, NULL, holder);
        if(holder[0] == NULL) return root;
        int t = holder[0]->data;
        holder[0]->data = holder[1]->data;
        holder[1]->data = t;
        return root;
    }
    struct Node* inorder(struct Node *curr, struct Node *prev, vector<Node*>&holder){
        if(!curr) return NULL;
        if(curr->left != NULL) prev = inorder(curr->left, prev, holder);
        ...
        prev = curr;
        if(curr->right != NULL) prev = inorder(curr->right, prev, holder);
        return prev;
    }
};
